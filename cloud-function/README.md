...
VK_USER_ID=числовой VK ID получателя
```

Токен сообщества: **Управление сообществом → Работа с API → Ключи доступа → Создать ключ** (право `messages`).
Получатель должен один раз написать сообществу (или включить «Сообщения сообщества» в настройках) — иначе VK не разрешит сообществу писать первым.
...
Если всё настроено правильно, вам придёт сообщение от сообщества в VK!
